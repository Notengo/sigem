...
$oAccion->setIdAccion($_GET['idAccion']);
$oAccion = $oMyAccion->find($oAccion);

$sql = "SELECT * FROM accionobjeto as a";
$sql .= " WHERE idAccion = " . $_GET['idAccion'];
if($_GET['criterio_usu_per'] <> '') {
    $sql .= " AND a.descripcion like '%".fn_filtro(substr(utf8_decode($_GET['criterio_usu_per']), 0, 16))."%' ";
}
