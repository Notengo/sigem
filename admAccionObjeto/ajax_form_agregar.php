...
<form action="javascript: fn_agregar();" method="post" id="frm_per">
    <input name="idAccion" type="hidden" id="idAccion" value="<?php echo $_POST['idAccion'];?>" />
    <table class="formulario">
        <tbody>
            <tr>
                <td>Objeto acci&oacute;n</td>
                <td><input name="accionObjeto" type="text" id="accionObjeto" size="50" class="required" />*</td>
            </tr>                   
        </tbody>
        <tfoot>
            <tr>
    ...
    $(document).ready(function(){
        $("#frm_per").validate({
            ...
            submitHandler: function(form) {
                var respuesta = confirm('\xBFDesea realmente agregar el objeto de acci&oacute;n?')
                if (respuesta)
                    form.submit();
                }
            });
