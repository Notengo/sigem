...
$sql = sprintf("SELECT * FROM accion WHERE idAccion = %d", (int)$_POST['ide_per']);

$per = mysql_query($sql);
$num_rs_per = mysql_num_rows($per);
if ($num_rs_per==0){
    echo "No existen sintomas con ese ID";
    exit;
}
...
?>
<h1>Modificaci&oacute;n de acciones</h1>
<p>Por favor rellene el siguiente formulario</p>
<form action="javascript: fn_modificar();" method="post" id="frm_per">
    <input type="hidden" id="idAccion" name="idAccion" value="<?=$rs_per['idAccion']?>" />
