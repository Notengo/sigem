...
<form action="javascript: fn_agregar();" method="post" id="frm_per">
    <table class="formulario">
        <tbody>
            <tr>
                <td>Pedido</td>
                <td><input name="pedido" type="text" id="pedido" size="50" class="required" />*</td>
            </tr>                   
        </tbody>
        <tfoot>
            <tr>
    ...
    $(document).ready(function(){
        $("#frm_per").validate({
            ...
            submitHandler: function(form) {
                var respuesta = confirm('\xBFDesea realmente agregar el pedido?')
                if (respuesta)
                    form.submit();
                }
            });
