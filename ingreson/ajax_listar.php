...
<thead>
    <tr><th></th>
        ...
        <th width="98px" id="boton"><center><a href="../generar" ><img src="../css/img_estilos/add.png" title="Nueva OT"></a>&nbsp;&nbsp;|&nbsp;&nbsp;
...
?>
    <tr id="tr_<?=$value->getNro()?>">
        ...
        <td >
            ...
            echo htmlentities($oTProblema->getDescripcion()." ".$oEspecialidades->getDescripcion()); 
            ?>
            <span style="width:550px;">
            <?php                                           
            echo "<img src='../images/valida.jpeg' />&nbsp;<cite>".html_entity_decode($value->getDescripcion())."</cite>";            
            ...
            $oHistorial->setNro($value->getNro());
            $aHistorial = $oMysqlHistorial->find($oHistorial);            
            if(count($aHistorial)>0) {    
                echo "<br/>";
                foreach ($aHistorial as $vervalue) {                    
                    echo $vervalue->getFechaInicio()."<br/>";
                    echo "<img src='../images/valida.jpeg' />&nbsp;<cite>".html_entity_decode($vervalue->getDescripcion())."</cite>";                    
                    $oUsuariosA = new UsuariosValueObject();
                    $oAgentesA = new AgentesValueObject();
                    $oUsuariosA->setId($vervalue->getUsuarioAlta());
                    $oUsuariosA=$oMysqlUsuarios->find($oUsuariosA); 
                    $oAgentesA->setDni($oUsuariosA->getDni());
                    $oAgentesA = $oMysqlAgentes->find($oAgentesA);
                    echo "<br/>Registrador: ".$oAgentesA->getNombre()."<br/>";                                        
                }
                 echo $value->getFechaInicio();
            }
            if($value->getObservacion()<>'')
                echo "<br/><br/>Obs.: <cite>".html_entity_decode($value->getObservacion())."</cite>";
            if($value->getEquipo()) {
        ...
        <td>
            ...
            <a href="javascript: fn_mostrar_frm_historial(<?=$value->getNro()?>);"><img src="../css/img_estilos/historial.gif" alt="Historial" title="Nueva entrada historial" /></a>                        
            <?php                 
