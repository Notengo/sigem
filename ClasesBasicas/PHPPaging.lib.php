	...
	function error($desarrollo = true, $msg, $query = false, $limit_error = null) {
		if($this->modo != 'publicacion' and $this->mostre_error == false) {
			if($this->modo == 'reporte') {
				...
				if($limit_error == true) $error.= "El error probablemente se deba a que en la consulta MySQL que usted indico aparentemente ya existia una clausula LIMIT, la cual es añadida automaticamente por el paginador. Mas informacion en la pagina web del script.<br /><br />";
				$error.= "Si no logra solucionar el problema, envie un mensaje a <code>phppaging@phperu.net</code> indicando el error mostrado y la consulta que genero el error";
			} else {
				$error = "Hubo un error ejecutando la paginaci�n. El mensaje devuelto es: <blockquote><code style='color: #080; font-size: 13px;'><strong>$msg</strong></code></blockquote>Si no logra solucionar el problema, env�e un mensaje a <code>phppaging@phperu.net</code> indicando el error mostrado y la consulta que genero el error";
			}
			$p = "<div style='border: 1px solid #666; background-color: #f6f6f6; margin: 5px 10px; padding: 10px 5px;'>";
			$p.= "<div style='color: #f00; font: 18px Georgia; margin: 0 5px; border-bottom: 1px dotted #f00;'><strong>Error Paginacion</strong></div>";
			$p.= "<div style='font: 12px Verdana; margin: 15px 19px;'>$error</div>";
			$p.= "<div style='font: 12px Georgia; text-align: right; color: #777; margin: 0; padding: 1px 4px;'></div></div>";
			echo $p;
		} else {
			$this->error = $msg;
		}
